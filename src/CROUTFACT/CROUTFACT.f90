...
subroutine diag(idiag,neq,n) 
      ...
      INTEGER, intent(Inout) :: idiag(neq)
       ! Locais
      INTEGER :: ieq, test
...
subroutine CroutFact_solver(nume, nd, neq, ngl, nnoel, numnp, Lm, stiff, u_n, u, N, T, Nflag, ALHS, CLHS, nALHS, ForceFact)
    ...
    !      Local
    INTEGER              :: iel, jnd, iCol, iLin, ieq, nALHS, innoel, ingl, N, Nflag, ForceFact
    INTEGER, allocatable :: lm_Aux(:,:), lm_Parab(:,:,:), idiag(:)
