subroutine NavierStokes3D_Solver
    ...
    CALL octa8_NavierStokes3D_Mass (stiff_Mass,x,y,z,incid,mtype,prop,numnp,nume,nummat,nd,nnoel,ncprop)
                  
    ! Prepara matriz de rigidez e difusao
    CALL octa8_NavierStokes3D_Difus (lm,x,y,z,incid,mtype,prop,numnp,nume,nummat,nwk,nd,neq+1,neqp,&
    nnoel,ncprop,stiff_difus_Lambda, stiff_difus_xMu,areatot)
    CALL octa8_NavierStokes3D_Convec (stiff,fp,up,u,lm,x,y,z,incid,mtype,prop,numnp,nume,nummat,nwk,&
    ...
    DO N = N0, NSTEPS
            ...
            u_n = f - fp*TFUNC - fbody - fpAcel
            
            K_x_u(:) = 0.d0                                                                          !
            CALL matvec ( u, K_x_u, ve1, ve2, neq, ielblk, nsize, nd, nelblk, nume, stiff, lm_aux )  ! Calcula K(u_n-2)*u_n-1                                                                                                                    !
            Normed_Ku_Ant = ddot(neq, K_x_u, 1, K_x_u, 1)
            K_x_u(:) = 0.d0                                                                          !
            CALL matvec ( u, K_x_u, ve1, ve2, neq, ielblk, nsize, nd, nelblk, nume, stiff_Convec, lm_aux )  ! Calcula K(u_n-2)*u_n-1                                                                                                                    !
            Normed_Ku_Conv_Ant = ddot(neq, K_x_u, 1, K_x_u, 1)
...
CHARACTER*70 :: filename1, filename2, filename3, filename4
            ...
            IF (MOD(N-1,nflag) .EQ. 0)  THEN                                 !
                  ...
                  call Calc_Pressure_Octa8(Pressure, TFUNC,N,T)
                  !call ensg_vec ('U',u,up,id,ngl,numnp,neq,neqp,kensight)    !
                  call OUTTECPLT(x,y,z,u,up,id,incid,ngl,numnp,nume,nnoel,neq,neqp,nd,kensight,T,TFUNC,ireordering,'U')
