...
/**
 * SZLOD variable data query service interface registered with Tecplot. The query service is
...
public:
    /**
     * Notifies the service prior to asking for any SZLOD data for this zone's variable. This
    ...
    /**
     * Notifies the service when the SZLOD data for this zone's variable is not currently needed.
    ...
    /**
     * Notifies the service when the SZLOD data for this zone's variable is no longer needed.
     ...
     * checkout() from code that has been made SZLOD addressing aware.
     * @param subzones
     ...
     */
    virtual void checkout(tecplot::RawArray<tecplot::ItemAddress::SzIndex_t> const& subzones) = 0;
     ...
     * checkin() when no longer needed. Tecplot calls checkin() from code that has been made SZLOD
     * addressing aware.
     * @param subzones
     *     Subzones for which the variable data are no longer needed.
     */
    virtual void checkin(tecplot::RawArray<tecplot::ItemAddress::SzIndex_t> const& subzones) = 0;
     ...
     * calls checkoutAll() from code that is not SZLOD addressing aware or for one reason or another
     * cannot utilize SZLOD addressing.
     * @throws std::bad_alloc if sufficient memory is not available
     * @throws tecplot::IOException if an I/O error occurs
     * @throws tecplot::InterruptedException if the process was interrupted
     */
     ...
     * made. Tecplot calls checkinAll() from code that is SZLOD addressing aware but for one reason
     * or another cannot utilize SZLOD addressing.
     */
