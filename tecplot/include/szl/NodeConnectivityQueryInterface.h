...
/**
 * SZLOD node connectivity query service interface registered with Tecplot. The query service is
...
public:
    /**
     * Notifies the service prior to asking for any SZLOD node connectivity for this zone. This
    ...
    /**
     * Notifies the service when the SZLOD node connectivity for this zone is not currently needed.
    ...
    /**
     * Notifies the service when the SZLOD node connectivity for this zone is no longer needed.
     ...
     * while still in use. Tecplot calls checkout() from code that has been made SZLOD addressing
     * aware.
     * @param nodeSubzones
     ...
     */
    virtual void checkout(tecplot::RawArray<tecplot::ItemAddress::SzIndex_t> const& nodeSubzones) = 0;
     ...
     *     longer needed.
     */
    virtual void checkin(tecplot::RawArray<tecplot::ItemAddress::SzIndex_t> const& nodeSubzones) = 0;
     ...
     * is not SZLOD addressing aware or for one reason or another cannot utilize SZLOD addressing.
     * @throws std::bad_alloc if sufficient memory is not available
     * @throws tecplot::IOException if an I/O error occurs
     * @throws tecplot::InterruptedException if the process was interrupted
     */
    ...
    virtual void referencedCellSubzones(
        tecplot::ItemAddress::SzIndex_t                     nodeSubzone,
        tecplot::RawArray<tecplot::ItemAddress::SzIndex_t>& cellSubzones) = 0;
     ...
     * and the size()-1.
     */
    virtual tecplot::ItemAddress::Offset_t size(tecplot::ItemAddress::SzIndex_t nodeSubzone) = 0;
