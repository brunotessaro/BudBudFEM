...
namespace tecplot {
class CellFaceNeighbors;
}

namespace tecplot { namespace szl {

/**
 * SZLOD cell connectivity query service interface registered with Tecplot. The query service is
...
public:
    ...
    /**
     * Notifies the service prior to asking for any SZLOD cell connectivity for this zone. This
    ...
    /**
     * Notifies the service when the SZLOD cell connectivity for this zone is not currently needed.
    ...
    /**
     * Notifies the service when the SZLOD cell connectivity for this zone is no longer needed.
    ...
    virtual void checkout(
        ...
        tecplot::RawArray<tecplot::ItemAddress::SzIndex_t> const& cellSubzones) = 0;
     ...
     * from code that has been made SZLOD addressing aware.
     * @param cellSubzones
     *     Cell subzones for which the connectivity and referenced node subzone indices are no
     *     longer needed.
     */
    virtual void checkin(tecplot::RawArray<tecplot::ItemAddress::SzIndex_t> const& cellSubzones) = 0;
     ...
     * is SZLOD addressing aware but for one reason or another cannot utilize SZLOD addressing.
     */
     ...
     * @see TecUtilDataNodeSetByRef(), TecUtilDataNodeArraySetByRef().
     * @param targetNM
    ...
    virtual void referencedNodeSubzones(
        tecplot::ItemAddress::SzIndex_t                     cellSubzone,
        tecplot::RawArray<tecplot::ItemAddress::SzIndex_t>& nodeSubzones) = 0;

    /**
     * Returns the number of cell subzones comprising the zone.
     */
    virtual tecplot::ItemAddress::SzIndex_t numSubzones() = 0;
     ...
     * and the size()-1.
     */
    virtual tecplot::ItemAddress::Offset_t size(tecplot::ItemAddress::SzIndex_t cellSubzone) = 0;
     ...
     * @throws std::bad_alloc if sufficient memory is not available
     */
    virtual void updateInternalFaceNeighbors(tecplot::RawArray<tecplot::ItemAddress::SzIndex_t> const& cellSubzones) = 0;
     ...
     * UnknownFaceNeighbor, is returned. For cell faces with definitive boundaries, a single
     * uniform address, NoCellFaceNeighbor, is returned. If the neighbors are cells, non-uniform
     * cell addresses are returned. The zone neighbors array contains the Tecplot zone neighbors
     * associated with each returned cell face neighbor or NoZoneFaceNeighbor indicating that a
     ...
     * @param neighbors
     *     Cell face neighbors instance to contain the results of the query.
     * @pre 0 <= cellFace < [cell type's max face count: 4 or 8]
     * @post neighbors.cellNeighbors().size() >= 1
     * @post neighbors.cellNeighbors().size() == neighbors.zoneNeighbors().size()
     * @post neighbors.cellNeighbors() contains UnknownFaceNeighbor -> neighbors.cellNeighbors().size() == 1
     * @post neighbors.cellNeighbors() contains NoCellFaceNeighbor -> neighbors.cellNeighbors().size() == 1
     */
    virtual void faceNeighbors(
        tecplot::ItemAddress              cellAddress,
        FaceIndex_t                       cellFace,
        class tecplot::CellFaceNeighbors& neighbors) = 0;

    /**
     * Assigns the cell face neighbors. If the neighbor is not yet known, as is the case initially
     * at a subzone boundary, a single uniform address, UnknownFaceNeighbor, should be
     * assigned.  If the face is a definitive boundary, a single uniform address,
     * NoCellFaceNeighbor, should be assigned. If the neighbors are cells they should be
     * non-uniform cell addresses. The zone neighbors array contains the Tecplot zone neighbors
     * associated with each supplied cell face neighbor or NoZoneFaceNeighbor indicating that a
     * cell face isn't connected to another zone. The loader is responsible for mapping between its
     * zone numbering and Tecplot's, therefore it must consider variable add/delete state changes
     * and update the mapping accordingly.
     * @param cellAddress
     ...
     * @param neighbors
     *     Instance containing the cell face neighbor information.
     * @pre 0 <= cellFace < [cell type's max face count: 4 or 8]
     * @pre cellNeighbors.size() >= 1
     * @pre cellNeighbors.size() == zoneNeighbors.size()
     * @pre cellNeighbors contains UnknownFaceNeighbor -> cellNeighbors.size() == 1
     * @pre cellNeighbors contains NoCellFaceNeighbor -> cellNeighbors.size() == 1
     * @throws std::bad_alloc if sufficient memory is not available
     */
    virtual void setFaceNeighbors(
        tecplot::ItemAddress                    cellAddress,
        FaceIndex_t                             cellFace,
        class tecplot::CellFaceNeighbors const& neighbors) = 0;
