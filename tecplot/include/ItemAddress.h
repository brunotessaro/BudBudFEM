...
public:
     ...
     *     an item addresses can be stored in an enumeration.
     */
    UInt64_t toRawBits() const
    {
        return m.rawBits;
        ENSURE(VALID_ITEM_ADDRESS(*this));
    }
     ...
     *     raw bits are exposed primarily so that an item addresses can be stored in an enumeration.
     */
